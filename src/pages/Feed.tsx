          ...
          {user ? (
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 text-sm font-golos"
                style={{ color: "#8b7355" }}
              >
                <div className="w-7 h-7 rounded-full bg-primary/20 flex items-center justify-center text-xs font-medium" style={{ color: "#8b7355" }}>
              ...
              {userMenuOpen && (
                <div className="absolute right-0 top-10 w-44 rounded-lg border shadow-lg py-1 z-50" style={{ backgroundColor: "#f5f0e8", borderColor: "#d4c9b0" }}>
                  <div className="px-4 py-2 text-xs font-golos border-b truncate" style={{ color: "#a0916e", borderColor: "#d4c9b0" }}>
                    {user.email}
                  </div>
                  {user.subscription && (
                    <div className="px-4 py-2 text-xs font-golos" style={{ color: "#8b7355" }}>
                      ✦ {user.subscription.tier.toUpperCase()}
                    </div>
                  )}
                  <button onClick={logout} className="w-full text-left px-4 py-2 text-sm font-golos hover:bg-black/5" style={{ color: "#8b7355" }}>
                    Sign out
                  </button>
                </div>
              )}
            ...
            <button
              ...
              className="text-sm font-golos px-4 py-1.5 border rounded-full transition-colors hover:bg-primary/10"
              style={{ color: "#8b7355", borderColor: "#d4c9b0" }}
            >
              Sign in
            </button>
          )}
    ...
    <div className="rounded-2xl overflow-hidden border shadow-sm" style={{ backgroundColor: "#fff8f0", borderColor: "#e8ddd0" }}>
      ...
      >
        {preview ? (
          <img
            ...
            style={item.locked ? { filter: "blur(12px) brightness(0.7)", transform: "scale(1.1)" } : {}}
          />
        ) : (
        ...
        {item.locked && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3">
            ...
            <button
              onClick={onSubscribe}
              className="px-5 py-2 text-xs font-golos tracking-widest uppercase rounded-full transition-all"
        ...
        {/* Expand icon */}
        {!item.locked && (
          <div className="absolute top-3 right-3 w-8 h-8 rounded-full flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity" style={{ backgroundColor: "rgba(0,0,0,0.4)" }}>
            <Icon name="Expand" size={14} style={{ color: "#fff" } as React.CSSProperties} />
          </div>
        )}
      </div>
      ...
      <div className="px-4 py-3 flex items-center justify-between">
        <div className="flex flex-col gap-0.5">
          {item.title && (
            <p className="font-cormorant text-base font-medium" style={{ color: "#5c4a32" }}>{item.title}</p>
          )}
          <p className="font-golos text-xs" style={{ color: "#b8a882" }}>
            {new Date(item.created_at).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}
          </p>
        </div>
        <div className="flex items-center gap-1.5 text-xs font-golos" style={{ color: "#b8a882" }}>
          {item.tier !== "free" && (
            <span className="px-2 py-0.5 rounded-full text-[10px] tracking-wider uppercase" style={{ backgroundColor: "#f0e8d8", color: "#8b7355" }}>
              {item.tier}
            </span>
          )}
        </div>
